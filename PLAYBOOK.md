# OpalCode Local Agent Playbook

This playbook is a practical guide to run OpalCode with a local model (Ollama), work safely, and get strong day-to-day results.
...
If everything is healthy, OpalCode starts directly.
...
Or let OpalCode recommend the best local model for your goal:
...
```powershell
Get-Content .\.opalcode-profile.json
```
...
- Keep any legacy workspace `.opalcode-profile.json` local; new provider saves use the user config directory.
