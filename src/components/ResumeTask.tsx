  ...
  if (loading) {
    return <Box flexDirection="column" padding={1}>
        <Box flexDirection="row">
          <Spinner />
          <Text bold>Loading OpalCode sessions…</Text>
        </Box>
        <Text dimColor>
          {retrying ? 'Retrying…' : 'Fetching your OpalCode sessions…'}
        </Text>
      </Box>;
  }
  if (loadErrorType) {
    return <Box flexDirection="column" padding={1}>
        <Text bold color="error">
          Error loading OpalCode sessions
        </Text>
  ...
  switch (errorType) {
    ...
    case 'other':
      return <Box marginY={1} flexDirection="row">
          <Text dimColor>Sorry, OpalCode encountered an error</Text>
        </Box>;
  }
}
