...
Manage the OpalCode project wiki stored in .opalcode/wiki.
...
function formatInitResult(result: Awaited<ReturnType<typeof initializeWiki>>): string {
  const lines = [`Initialized OpalCode wiki at ${result.root}`]
...
function formatStatus(status: Awaited<ReturnType<typeof getWikiStatus>>): string {
  if (!status.initialized) {
    return `OpalCode wiki is not initialized in this project.\n\nRun /wiki init to create ${status.root}.`
  }

  return [
    'OpalCode wiki status',
    '',
    `Root: ${status.root}`,
...
function formatIngestResult(
  ...
  return [
    `Ingested ${result.sourceFile} into the OpalCode wiki.`,
    '',
    `Title: ${result.title}`,
