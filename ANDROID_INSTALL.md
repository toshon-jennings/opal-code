# OpalCode on Android (Termux)

A complete guide to running OpalCode on Android using Termux + proot Ubuntu.
...
OpalCode requires [Bun](https://bun.sh) to build, and Bun does not support Android natively. The workaround is running a real Ubuntu environment inside Termux via `proot-distro`, where Bun's Linux binary works correctly.
...
### Step 6 — Build OpalCode

```bash
cd /data/data/com.termux/files/home/opalcode
bun run build
```

You should see:
```
✓ Built opalcode v0.1.6 → dist/cli.mjs
```
...
Both were tested and fail due to OpalCode's large system prompt (~50K tokens):
...
- Run `bun run build` again only if you pull updates to the OpalCode repo.
