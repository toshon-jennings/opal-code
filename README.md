...
OpalCode is an open-source coding-agent CLI for cloud and local model providers.
...
OpalCode is also mirrored to GitLawb:
[gitlawb.com/node/repos/z6MkqDnb/opalcode](https://gitlawb.com/node/repos/z6MkqDnb/opalcode)
...
If the install later reports `ripgrep not found`, install ripgrep system-wide and confirm `rg --version` works in the same terminal before starting OpalCode.
...
```bash
ollama launch opalcode --model qwen2.5-coder:7b
```
...
| Codex | `/provider` | Uses existing Codex CLI auth, OpalCode secure storage, or env credentials |
...
OpalCode supports multiple providers, but behavior is not identical across all of them.

- Anthropic-specific features may not exist on other providers
- Tool quality depends heavily on the selected model
- Smaller local models can struggle with long multi-step tool flows
- Some providers impose lower output caps than the CLI defaults, and OpalCode adapts where possible

For best results, use models with strong tool/function calling support.

## Agent Routing

OpalCode can route different agents to different models through settings-based routing. This is useful for cost optimization or splitting work by model strength.

Add to `~/.opalcode.json`:
...
For Anthropic-native backends and Codex responses, OpalCode keeps the native provider web search behavior.
...
OpalCode can be run as a headless gRPC service, allowing you to integrate its agentic capabilities (tools, bash, file editing) into other applications, CI/CD pipelines, or custom user interfaces. The server uses bidirectional streaming to send real-time text chunks, tool calls, and request permissions for sensitive commands.
...
*Note: The gRPC definitions are located in `src/proto/opalcode.proto`. You can use this file to generate clients in Python, Go, Rust, or any other language.*
...
OpalCode uses Bun's built-in test runner for unit tests.
...
Coverage output is written to `coverage/lcov.info`, and OpalCode also generates a git-activity-style heatmap at `coverage/index.html`.
...
- `vscode-extension/opalcode-vscode/` - VS Code extension
- `.github/` - repo automation, templates, and CI configuration
- `bin/` - CLI launcher entrypoints

## VS Code Extension

The repo includes a VS Code extension in [`vscode-extension/opalcode-vscode`](vscode-extension/opalcode-vscode) for OpalCode launch integration, provider-aware control-center UI, and theme support.
...
- Use [GitHub Discussions](https://github.com/Gitlawb/opalcode/discussions) for Q&A, ideas, and community conversation
- Use [GitHub Issues](https://github.com/Gitlawb/opalcode/issues) for confirmed bugs and actionable feature work
