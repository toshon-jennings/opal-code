...
function createStatus(overrides = {}) {
  return {
    ...
    shimEnabled: false,
    workspaceFolder: '/workspace/opalcode/very/long/path/example-project',
    workspaceSourceLabel: 'active editor workspace',
    launchCwd: '/workspace/opalcode/very/long/path/example-project',
    launchCwdLabel: '/workspace/opalcode/very/long/path/example-project',
    canLaunchInWorkspaceRoot: true,
    profileStatusLabel: 'Found',
    profileStatusHint: '/workspace/opalcode/very/long/path/example-project/.opalcode-profile.json',
    workspaceProfilePath: '/workspace/opalcode/very/long/path/example-project/.opalcode-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
...
test('renderControlCenterHtml uses the OpalCode wordmark, status rail, and warm action hierarchy', () => {
  ...
  assert.match(
    html,
    /title="\/workspace\/opalcode\/very\/long\/path\/example-project"[^>]*>\/workspace\/opalcode\/very\/long\/path\/example-project<\//,
  );
});
...
test('OpalCodeControlCenterProvider.getHtml supplies a nonce to the redesigned renderer', () => {
  const { OpalCodeControlCenterProvider } = loadExtension();
...
test('resolveLaunchTargets distinguishes project-aware launch from workspace-root launch', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      activeFilePath: '/workspace/opalcode/src/panels/control-center.js',
      workspacePath: '/workspace/opalcode',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/opalcode/src/panels',
      projectAwareCwdLabel: '/workspace/opalcode/src/panels',
      projectAwareSourceLabel: 'active file directory',
      workspaceRootCwd: '/workspace/opalcode',
      workspaceRootCwdLabel: '/workspace/opalcode',
...
test('resolveLaunchTargets anchors relative launch commands to the workspace root', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      executable: './node_modules/.bin/opalcode',
      activeFilePath: '/workspace/opalcode/src/panels/control-center.js',
      workspacePath: '/workspace/opalcode',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/opalcode',
      projectAwareCwdLabel: '/workspace/opalcode',
...
test('renderControlCenterHtml explains distinct launch targets when an active file directory is available', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/opalcode/src/panels',
      launchCwdLabel: '/workspace/opalcode/src/panels',
  ...
  assert.match(html, /Starts beside the active file · \/workspace\/opalcode\/src\/panels/);
  assert.match(html, /Always starts at the workspace root · \/workspace\/opalcode/);
});
...
test('renderControlCenterHtml makes shared workspace-root launches explicit for relative commands', () => {
  ...
  assert.match(html, /Project-aware launch is anchored to the workspace root by the relative command · \/workspace\/opalcode/);
  assert.match(html, /Same workspace-root target as Launch OpalCode because the relative command resolves from the workspace root · \/workspace\/opalcode/);
});
